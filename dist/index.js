...
var require_dist_node2 = __commonJS({
  "node_modules/@octokit/endpoint/dist-node/index.js"(exports2) {
    ...
    function endpointWithDefaults(defaults, route, options) {
      return parse6(merge(defaults, route, options));
    }
...
var require_args = __commonJS({
  "node_modules/gm/lib/args.js"(exports2, module2) {
    var argsToArray = require_utils5().argsToArray;
    var isUtil = require_utils5().isUtil;
...
var require_drawing = __commonJS({
  "node_modules/gm/lib/drawing.js"(exports2, module2) {
    var escape = require_utils5().escape;
    module2.exports = function(proto) {
...
var require_array_parallel = __commonJS({
  "node_modules/array-parallel/index.js"(exports2, module2) {
    module2.exports = function parallel(fns, context2, callback) {
      if (!callback) {
      ...
      fns.forEach(context2 ? function(fn, i) {
        fn.call(context2, maybeDone(i));
      } : function(fn, i) {
        fn(maybeDone(i));
      });
    ...
    module2.exports = function(proto) {
      ...
      proto.morph = function morph(other, outname, callback) {
        ...
        self.write(outname, function(err, stdout, stderr, cmd) {
          ...
          fs5.exists(outname, function(exists) {
            if (exists) return callback(null, stdout, stderr, cmd);
            parallel([
              fs5.unlink.bind(fs5, outname + ".0"),
              fs5.unlink.bind(fs5, outname + ".2"),
              fs5.rename.bind(fs5, outname + ".1", outname)
            ], function(err2) {
...
var require_cross_spawn = __commonJS({
  "node_modules/cross-spawn/index.js"(exports2, module2) {
    ...
    function spawn(command, args, options) {
      var parsed;
      var spawned;
      parsed = parse6(command, args, options);
...
var require_array_series = __commonJS({
  "node_modules/array-series/index.js"(exports2, module2) {
    module2.exports = function series(fns, context2, callback) {
      if (!callback) {
        if (typeof context2 === "function") {
          callback = context2;
          context2 = null;
      ...
      fns = fns.slice(0);
      var call = context2 ? function() {
        fns.length ? fns.shift().call(context2, next) : callback();
      } : function() {
...
var require_gm = __commonJS({
  "node_modules/gm/index.js"(exports2, module2) {
    ...
    module2.exports.utils = require_utils5();
...
var generateJson = async (buildDir, repoUrl, images) => {
...
var generateMarkdown = async (buildDir, images) => {
  const lines = [
    "| Map | Course | Thumbnail | Full | Medium | Thumb | webp Full | webp Medium | webp Thumb |",
    "|-----|--------|-----------|------|--------|-------|-----------|-------------|------------|"
  ];
  images.forEach((image) => {
    const map = path2.basename(path2.dirname(image));
    const course = path2.parse(image).name;
    const line = [
      `|${map}`,
      `|${course}`,
  ...
  const filePath = path2.join(buildDir, "README.md");
  return fs2.promises.appendFile(filePath, lines.join("\n"));
};
...
var ensureDir = async (dir) => {
  return fs3.promises.mkdir(dir, { recursive: true });
};
...
var getFilesInDir = async (dir, validExtensions2) => {
  const files = await fs3.promises.readdir(dir, { recursive: true });
  const validFiles = files.filter((file) => {
    const parsed = path3.parse(file);
    return validExtensions2.includes(parsed.ext);
  });
...
var ImageService = class {
  ...
  async generateImageVariant(srcImage, format, variant) {
    ...
    return this.resizeImage(path4.resolve(srcImage.filepath), path4.resolve(destImage), dimensions);
  }
};

// src/constants.ts
var validEvents = ["push", "workflow_dispatch"];
var validExtensions = [".jpg", ".jpeg", ".webp"];
var removalStatuses = ["removed"];
var generateStatuses = ["added", "changed", "modified", "renamed"];

// src/index.ts
var run = async () => {
  const token = core2.getInput("token");
  const srcDir = core2.getInput("src_dir");
  const buildDir = core2.getInput("build_dir");
  const context2 = github.context;
  const repoUrl = context2.payload.repository?.html_url;
  const event = context2.eventName;
  if (!validEvents.includes(event)) {
    return core2.setFailed(`Invalid event ${event}`);
  }
  if (!repoUrl) {
    return core2.setFailed("Failed to retrieve repository url");
  }
  if (context2.ref !== "refs/heads/master") {
    return core2.setFailed("This action can only be invoked on master");
  }
  core2.info(`Received event ${event}`);
  const toRemove = [];
  const toGenerate = [];
  ...
  if (event === "push") {
    ...
    if (!files) {
      return core2.setFailed("Could not get diff");
    }
    const imageFiles = files.filter((f) => {
      const parsed = path5.parse(f.filename);
    ...
    toRemove.push(
      ...removed.map((f) => {
        const map = path5.basename(path5.dirname(f.filename));
        return { map, name: path5.parse(f.filename).name, filepath: f.filename };
      })
    );
    toGenerate.push(
      ...modified.map((f) => {
